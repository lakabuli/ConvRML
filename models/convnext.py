...
class Decoder(nn.Module):
    def __init__(self, sum_encoder_embed_dims=1440, rec_channels=3, original_height=300, original_width=480):  # convnext: 96 + 192 + 384 + 768 = 1440
...
class ConvRecon(nn.Module):
    def __init__(self, n_channels, height, width, model_size="tiny", pretrained=False, in_22k=False, drop_path_rate=0.2, layer_scale_init_value=1.0):
        super().__init__()
        if model_size == "tiny":
            ...
            self.encoder = convnext_small(pretrained, in_22k, drop_path_rate=drop_path_rate, layer_scale_init_value=layer_scale_init_value, in_chans=n_channels)
